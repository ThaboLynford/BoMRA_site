...
abstract class YasrShortcode {
    ...
    public function enqueueScripts() {
        ...
        if($called_class === 'YasrOverallRating' || $called_class === 'YasrMultiSet'
            || $called_class === 'YasrVisitorMultiSet') {
            wp_enqueue_script(
                'yasr-ov-multi', YASR_JS_DIR_INCLUDES . 'shortcodes/overall-multiset.js',
                array('jquery', 'rater', 'wp-i18n'), YASR_VERSION_NUM, true
            );
        }
        ...
        if($called_class === 'YasrRankings' || $called_class === 'YasrNoStarsRankings') {
            wp_enqueue_script(
                ...
                array('jquery', 'rater', 'wp-i18n', 'wp-element'),
                YASR_VERSION_NUM,
                true
            );
        }
    ...
    protected function starSize() {
        ...
        if ($this->shortcode_name === 'yasr_ov_ranking'
            || $this->shortcode_name === 'yasr_most_or_highest_rated_posts'
            || $this->shortcode_name === 'yasr_pro_ur_ranking'
            || $this->shortcode_name === 'yasr_multi_set_ranking'
            || $this->shortcode_name === 'yasr_visitor_multi_set_ranking'
        ) {
            //default size for all rankings
            $size = 'medium';
            if (yasr_fs()->is__premium_only()) {
                if (yasr_fs()->can_use_premium_code()) {
                    $size = apply_filters('yasr_size_ranking', 'medium');
                }
            }
        }
