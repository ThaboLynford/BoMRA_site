...
class YasrProCRIncludes {
    ...
    public function urRankingReview ($atts, $content, $shortcode_tag) {
        ...
        $ranking_comment->query_result_most_rated_visitor    = self::rankingsByCommentsData($atts, 'most');
        $ranking_comment->query_result_highest_rated_visitor = self::rankingsByCommentsData($atts, 'highest');

        $ranking_comment->returnDoubleTableRanking($ranking_comment->urlencodeAtts($atts), $source);
