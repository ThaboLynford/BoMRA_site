...
class JuMainQueue
{
    ...
    public function checkQueueExist($value = '', $compare = '=')
    {
        ...
        switch ($compare) {
            case '=':
                $row = $wpdb->get_row($wpdb->prepare('SELECT id, status FROM ' . $table . ' WHERE datas = BINARY %s ORDER BY date_added DESC', array($value)));
                break;
            case 'LIKE':
                $row = $wpdb->get_row($wpdb->prepare('SELECT id, status FROM ' . $table . ' WHERE datas LIKE BINARY %s ORDER BY date_added DESC', array('%' . $value . '%')));
                break;
            default:
                $row = $wpdb->get_row($wpdb->prepare('SELECT id, status FROM ' . $table . ' WHERE datas = BINARY %s ORDER BY date_added DESC', array($value)));
        }
    ...
    private function proceedQueue()
    {
        ...
        // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Params has prepared
        $wpdb->query('DELETE FROM ' . $wpdb->prefix . self::getOption('plugin_prefix') . '_queue WHERE date_done < (UNIX_TIMESTAMP()*1000 - 7 * 24 * 60 * 60 * 1000)');
