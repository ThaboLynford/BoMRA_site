...
class Wow_Plugin_Admin {
	...
	public function pro( $tooltip = null ) {
		...
		$title   = esc_attr__( 'More features in the PRO version', 'floating-button' );
	...
	public function rate_us( $footer_text ) {
		...
		if ( $wow_plugin_page == $this->plugin['slug'] ) {
			$rate_text = sprintf( __( 'Thank you for using <a href="%1$s" target="_blank">' . esc_attr( $this->plugin['name'] )
			                          ...
			                          . esc_attr( $this->rating['website'] ) . '</a>', 'floating-button' ), $this->url['home'],
				$this->rating['url'] );
	...
	public function save_data() {
		...
		$response = array(
			'status'  => 'NO',
			'message' => esc_attr__( 'Something went wrong. Contact the plugin developer', 'floating-button' ),
		);
		if ( 1 === $add ) {
