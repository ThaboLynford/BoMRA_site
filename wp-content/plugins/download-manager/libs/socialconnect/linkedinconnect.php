	...
	function login(){
        if(wpdm_query_var('sociallogin') == 'linkedin'){
            if(!isset($_GET['code'])) {
                ...
                $client = new Client(
                    get_option('_wpdm_linkedin_client_id'),
                    get_option('_wpdm_linkedin_client_secret')
                );


                $token = $client->getAccessToken(wpdm_query_var('code'));
                $token_expires = $client->getAccessTokenExpiration();

                $user = $client->get(    'people/~:(id,email-address,first-name,last-name)');
                wpdmdd($user);
