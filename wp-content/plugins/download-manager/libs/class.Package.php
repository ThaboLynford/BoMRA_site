...
//*/
    ...
    public static function userDownloadLimitExceeded($ID){
        ...
        $user_download_count = maybe_unserialize(get_post_meta($ID, "__wpdmx_user_download_count", true));
        $user_download_count = isset($user_download_count[$index])?$user_download_count[$index]:0;
        ...
        $result = apply_filters("wpdm_user_download_limit_exceeded", false, $ID);
...
//                        $adata .= '<div class="modal fade" role="modal" id="pkg_' . $package['ID'] . "_" . $unqid . '"><div class="modal-dialog"><div class="modal-content"><div class="modal-header"><strong style="margin:0px;font-size:12pt">' . __('Download') . '</strong></div><div class="modal-body">' . $data . '</div><div class="modal-footer text-right"><button type="button" class="btn btn-secondary btn-sm" data-dismiss="modal">Close</button></div></div></div></div>';
    ...
    private static function activeLocks($package, $params = array('embed'=>0, 'popstyle' => 'pop-over')){
        ...
        if($sociallock!=""){
            ...
            $data .= "<div class='panel panel-default card card-default '><div class='panel-heading card-header'>".get_option('_wpdm_social_lock_panel_title', 'Like or Share to Download')."</div><div class='panel-body card-body wpdm-social-locks text-center'>{$socdesc}{$sociallock}</div></div>";
        }
    ...
    public static function downloadLink($ID, $embed = 0, $extras = array())
    {
        ...
        if (!@in_array('guest', @maybe_unserialize($package['access'])) && !is_user_logged_in()) {
            ...
            if (get_option('_wpdm_hide_all', 0) == 1) {
                $hide_all_message = get_option('__wpdm_login_form', 0) == 1 ? $loginform : stripcslashes(str_replace(array("[loginform]","[this_url]", "[package_url]"), array($loginform, $_SERVER['REQUEST_URI'],get_permalink($package['ID'])), $loginmsg));
            ...
            $package['download_link'] = $package['download_link_extended'] = $package['download_link_popup'] = stripcslashes(str_replace(array("[loginform]","[this_url]", "[package_url]"), array($loginform, $_SERVER['REQUEST_URI'],get_permalink($package['ID'])), $loginmsg));
            return get_option('__wpdm_login_form', 0) == 1 ? $loginform : $package['download_link'];
     ...
     * @return string
     */
    public static function getDownloadURL($ID, $ext = array()){
        ...
        if($flat) $download_url = home_url("/download/{$ID}-{$code}/{$filename}");
        return $download_url;
    }
     ...
     * @return string
     */
    static function expirableDownloadLink($ID, $usageLimit = 10, $expirePeriod = 999999, $sessionOnly = true){
        $key = uniqid();
        ...
        $download_url = add_query_arg(array("wpdmdl" => $ID, "_wpdmkey" => $key), get_permalink($ID));
        return $download_url;
    }
    ...
    public static function fetchTemplate($template, $vars, $type = 'link')
    {
        ...
        if(isset($templates[$template]) && isset($templates[$template]['content'])) $template = $templates[$template]['content'];
            ...
            if(!strpos(strip_tags($template), "]")){
                ...
                if (file_exists($themeltpldir . $template)) $template = file_get_contents($themeltpldir . $template); // Apply if available in the child theme
        ...
        if ($type == 'page' && (strpos($template, '[similar_downloads]') || strpos($vars['description'], '[similar_downloads]')))
            $vars['similar_downloads'] = wpdm_similar_packages($vars, 6);
    ...
    public static function docPreview($package){
        ...
        $url = self::getDownloadURL($package['ID'], $params);
        $url .= "&open=1";
