	    ...
	    if( count($files) > 2 ) { /* The 2 accounts for . and .. */
		    ...
		    foreach( $files as $file ) {
			    if( $file != '.' && $file != '..' && is_dir($root . $_REQUEST['dir'] . $file) ) {
				    echo "<div class=\"col-md-4\"><a class='dir' onclick=\"explore('".realpath( $root .'/'. $_REQUEST['dir'] .'/'. $file )."', this)\" class=\"odir\" href=\"#\" rel=\"" . htmlentities(realpath($_REQUEST['dir'] .'/'. $file)) . "/\"><i class=\"fa fa-folder-open\"></i> &nbsp;" . htmlentities($file) . "</a></div>";
			    }
		    }
	    }
    }
