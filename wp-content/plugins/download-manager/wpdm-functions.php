...
function wpdm_getlink()
{
    ...
    if (isset($_POST['verify']) && $_POST['verify'] == 'email' && $file['email_lock'] == 1) {
        if (wpdm_verify_email($_POST['email'])) {
            ...
            $download_url  = \WPDM\Package::expirableDownloadLink($id, $limit);
            $download_page_url  = \WPDM\Package::expirableDownloadPage($id, $limit);
            ...
            if($idl === 0 || ($idl == 1 && $idle == 0)) {
                ...
                \WPDM\Email::send("email-lock", array('to_email' => $_POST['email'], 'name' => $name, 'download_count' => $limit,  'package_name' => $file['post_title'], 'package_url' => get_permalink($id), 'download_url' => $download_page_url));
...
function wpdm_embed_category($params = array('id' => '', 'operator' => 'IN' , 'items_per_page' => 10, 'title' => false, 'desc' => false, 'order_field' => 'create_date', 'order' => 'desc', 'paging' => false, 'toolbar' => 1, 'template' => '','cols'=>3, 'colspad'=>2, 'colsphone' => 1, 'morelink' => 1))
{
    ...
    $orderby = isset($_GET['orderby']) && in_array($_GET['orderby'], array('title', 'publish_date', 'updates', 'download_count', 'view_count')) ? esc_attr($_GET['orderby']) : 'publish_date';
    $order_field = (!isset($_GET['orderby']) && isset($order_field))? $order_field : 'publish_date';
...
add_filter("send_download_link_to_label", function($label){
